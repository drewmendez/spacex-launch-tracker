...
export const useLaunches = (filter: Ref<string | null, string | null>, sortOrder: Ref<string, string>) => {
	...
	const launchesData = computed(() => data.value?.launches ?? [])

	const launches = computed(() => {
		if (!filter.value) {
			return transformData(sortByDate(launchesData.value, sortOrder.value))
		} else {
			return transformData(
				sortByDate(
					launchesData.value.filter((launch) => launch.launch_year === filter.value),
					sortOrder.value,
				),
			)
		}
